...
class LoginModel{
    ...
    public function deletePermission($ma_quyen){
        $query = "DELETE FROM quyen WHERE ma_quyen = $ma_quyen and ma_quyen NOT IN (SELECT ma_quyen FROM taikhoan);";
        $result = $this->db->execute($query);
        if($result){
            $query2 = "DELETE FROM ct_phan_quyen where ma_quyen = $ma_quyen";
            return $this->db->execute($query2);
        }
        return false;
    }
    public function updatePermission($maquyen,$ten_quyen){
        $sql = "UPDATE quyen set ten_quyen = '$ten_quyen' where ma_quyen = '$maquyen'";
        return $this->db->execute($sql);
    }
    public function updateDetail($ma_quyen, $ma_cn,$them,$sua,$xoa){
        $sql = "UPDATE ct_phan_quyen set them = $them, sua = $sua, xoa = $xoa where ma_quyen = $ma_quyen and ma_cn = $ma_cn";
        return $this->db->execute($sql);
    }
